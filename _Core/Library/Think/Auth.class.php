...
class Auth{
    ...
    public function getGroups($uid) {
        ...
        $user_groups = M()
            ...
            ->table($this->_config['AUTH_USER'] . ' a')
            ->where("a.id='$uid' and g.status='1'")
            ->join($this->_config['AUTH_GROUP']." g on a.role_id=g.id")
            ->field('a.id as  uid,a.role_id as group_id,g.title,g.rules')->select();
    ...
    protected function getAuthList($uid,$type) {
        ...
        $rules_o = M()->table($this->_config['AUTH_RULE'])->where($map2)->field('condition,name')->select();
		
		// echo M()->getLastsql();
		if($rules && $rules_o){
			$rules = array_merge($rules,$rules_o);
		}elseif(!$rules && $rules_o){
			$rules = $rules_o;
		}
		// print_r($rules);
        //循环规则，判断结果。

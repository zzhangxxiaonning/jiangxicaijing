...
<section class="panel" style="margin-bottom:0px;">
	<header class="panel-heading">
		...
		<span class="tools pull-right">
			...
			<button class="btn btn-info" type="button" onclick="location.href='<?php echo U('stu_add');?>'"><i class="fa fa-plus"></i> 添加</button>
		</span>

	</header>
	<div class="panel-body" style=" <?php if(($is_service) == "0"): ?>display: none;<?php else: ?>display: block;<?php endif; ?>">
		<form action="<?php echo U('stu_index');?>" method="get">
		<div class="col-md-2" >
			<div class="input-group panel">
				<span class="input-group-addon">名称</span>
				<input type="text" name="name" value="<?php echo I('get.name');?>" class="form-control" placeholder="姓名/学号">
			</div>
		</div>
			<div class="col-md-2">
				<div class="input-group panel">
...
<table class="table table-bordered">
	...
	<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
		...
		<td>
			<?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Student/editnav',array('id'=>$vo['id']));?>" class="label label-success label-mini">已启用</a>
			<?php else: ?>
				<a href="<?php echo U('Student/editnav',array('id'=>$vo['id']));?>" class="label label-warning label-mini">已禁用</a><?php endif; ?>
		</td>
		<td><?php echo (date('Y-m-d H:i:s',$vo["create_time"])); ?></td>
		<td>
		<a href="<?php echo U('stu_edit',array('id'=>$vo['id']));?>" class="btn btn-default btn-sm" id="add-sticky"><i class="fa fa-edit"></i> 编辑</a>
				...
				<div class="">
					<form action="<?php echo U('impUser');?>" method="post" enctype="multipart/form-data">
						<span style="float:left;">
							<button class="btn btn-info-back" type="button" onclick="location.href='<?php echo U('expUser',array('name'=>I('name'),'status'=>I('status'),'start_time'=>I('start_time'),'end_time'=>I('end_time')));?>'">导出</button>

...
<section class="panel" style="margin-bottom:0px;">
	<header class="panel-heading">
		...
		<span class="tools pull-right">
			...
			<button class="btn btn-info" type="button" onclick="location.href='<?php echo U('add');?>'"><i class="fa fa-plus"></i> 普通发送</button>
			<button class="btn btn-info" type="button" onclick="location.href='<?php echo U('adds');?>'"><i class="fa fa-plus"></i> 群发</button>
		</span>
	</header>
	<div class="panel-body" style=" <?php if(($is_service) == "0"): ?>display: none;<?php else: ?>display: block;<?php endif; ?>">
<form action="<?php echo U('index_mail');?>" method="get">
<div class="col-md-2"">
	<div class="input-group panel">
		<span class="input-group-addon">标题</span>
		<input type="text" name="title" value="<?php echo I('get.title');?>" class="form-control" placeholder="">
	</div>
</div>
	...
	<div class="col-md-2">
		<div class="input-group panel">
			...
			<select class="form-control" name="send_type">
				<option value="-1">全部</option>
				<option value="0" <?php if(I('get.send_type','-1') == 1): ?>selected<?php endif; ?> >普通</option>
				<option value="1" <?php if(I('get.send_type','-1') == 1): ?>selected<?php endif; ?> >群发</option>
			</select>
		</div>
	</div>
<div class="col-md-3">
...
<table class="table table-bordered">
	...
	<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
		...
		<td>
			<?php if($vo['status'] == '1'): ?><a href="javascript:void(0)" class="label label-success label-mini">已查看</a>
			<?php elseif($vo['status'] == '0'): ?>
				<a href="javascript:void(0)" class="label label-warning label-mini">未查看</a>
			<?php else: ?>
				<a href="javascript:void(0)" class="label label-mini" style="color:black;">群发</a><?php endif; ?>
		</td>
		<td><?php echo ($vo["admin_name"]); ?><br></td>
		<td><?php echo (date('Y-m-d H:i:s',$vo["create_time"])); ?></td>
		<td>

			<a href="<?php echo U('edit_mail',array('id'=>$vo['id']));?>" class="btn btn-default btn-sm"><i class="fa fa-eye"></i>查看</a>
			<a href="javascript:void(0);" class="btn btn-default btn-sm jQ_delete_reload" data-href="<?php echo U('del_index',array('id'=>$vo['id']));?>" title="取得是否删除“<?php echo ($vo["username"]); ?>”的站内信"><i class="fa fa-times"></i> 删除</a>
			<a href="<?php echo U('editnav',array('id'=>$vo['id']));?>" class="btn btn-default btn-sm"><i class="fa fa-edit"></i>
			...
			<div class="panel-body">
				...
				<h4>Tips:</h4>
				<p>1.状态：<code>已查看</code>表示该信息用户已经查看；<code>未查看</code>表示该信息用户暂未查看；</p>

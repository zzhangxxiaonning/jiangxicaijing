...
use Think\Model;
Class ActattrModel extends Model{
	public $sign = '';
    ...
    public function getList( $cid = 0, $space = "",$orderby = 'sort asc,id asc',$condition = NULL,$is=0) {
        if($is==0 && $cid!=0){
            $a=M('Actattr')->field('id,up_id,path,catname')->where('id='.$cid)->select();
            foreach($a as $k=>$v){
                $a[$k]['tname']=$v['catname'];
            }
    ...
    */
    ...
    public function getChild($fid=0,$orderby = 'sort asc,id asc',$condition) {
        ...
        $childs=M('Actattr')->field('id,up_id,path,catname')->where($condition)->order($orderby)->select();
        return $childs;
    }
}

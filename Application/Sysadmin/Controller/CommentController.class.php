...
class CommentController extends CommonController {
    ...
    public function index(){
		...
		if($name){
            ...
            $news_1 = M("Activity")->where($map_1)->select();
            $str_1 = '';
	...
	public function edit(){
		...
		if(IS_POST){
			...
			if(!$_db->create($_POST)){
				...
				if($ret){
					$this->inserlog('活动评论ID:'.$id.' 修改成功','Comment');
					$this->success("修改成功",get_up_url(U('index'),'Comment'));
				}else{
					$this->error("修改失败");
				}
			}
...
//                $lastids = $_db_1 -> where($map_cr_1) -> save($data_cr);
    ...
    public function editcom(){
        ...
        if(IS_GET){
            ...
            if($lastid){
                $this->inserlog('活动评论ID:'.$id.' 修改通过状态成功','Comment');
                $this->success("修改通过状态成功",get_up_url(U('index'),'Comment'));
            }else{
                $this->error("修改通过状态失败");
            }
    ...
    public function editnav(){
        ...
        if(IS_GET){
            ...
            $lastid=$Mail->where($map)->save($saveda);
            if($lastid){
                $this->inserlog('活动评论ID:'.$id.' 修改显示状态成功','Comment');
                $this->success("修改显示状态成功",get_up_url(U('index'),'Comment'));
            }else{
                $this->error("修改显示状态失败");
            }
    ...
    public function del_com(){
        if(IS_GET){
            ...
            if($ret){
                $json['status'] = '1';
                $this->inserlog('用户'.$username.'活动评论ID:'.$id.' 删除成功','Comment');
                $json['info']	='删除成功';
            }else{
                $json['status'] = '0';
                $json['info']	= '删除失败';

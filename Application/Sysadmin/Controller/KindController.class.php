...
class KindController extends CommonController{
	...
	public function __construct() {
		...
		$this->php_path = $_SERVER['DOCUMENT_ROOT'].UPLOADS_PATH;
...
//            'ads'	=> array('gif', 'jpg', 'jpeg', 'png', 'bmp'),
		...
		if (empty($_FILES) === false) {
			...
			if (@is_dir($save_path) === false) {
				$this->alert("上传目录不存在。".$_SERVER['DOCUMENT_ROOT']);
			}
			//检查目录写权限
			...
			$json = new \Common\Other\Services_JSON();
			echo $json->encode(array('error' => 0, 'url' => $file_url,'new_name'=>$temp_arr[0]));
			exit;
		}
	}

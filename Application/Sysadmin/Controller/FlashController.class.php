...
class FlashController extends CommonController {
    ...
    public function rec_index(){
		create_up_url(U('rec_index'),'Flash');
		$cat_name = '直播管理';
		$_db = M('Flash');
		$map = array();
		$is_service = 0;
		...
		if(IS_POST){
			...
			if($datas){
				...
				if(!$_db->create()){
					...
					$lastid = $_db->add();
					if($lastid){
						$this->inserlog('直播：'.I('post.title').',ID:'.$lastid.' 添加成功','Flash');
						$this->success("直播：".I('post.title')."添加成功",get_up_url(U('rec_index'),'Flash'));
					}else{
						$this->error("直播：".I('post.title')."添加失败");
					}
				}
			}	
			...
			if($datas){
				...
				if(!$_db->create($_POST)){
					...
					if($ret){
						...
						$this->inserlog('直播：'.I('post.title').',ID:'.$id.' 编辑成功','Flash');
						$this->success("直播：".I('post.title')."编辑成功",get_up_url(U('rec_index'),'Flash'));
					}else{
						$this->error("直播：".I('post.title')."编辑失败");
					}
				}
			}
		}else{
...
//            is_resource($info['content']) && $info['content']=stream_get_contents($info['content']);
	...
	public function del_index(){
		if(IS_GET){
			...
			$info = M('Flash')->where($m_admin)->find();
			$ret = M('Flash')->where($m_admin)->limit(1)->delete();
//            $sql = "DELETE FROM cms_circle WHERE id = ".$id;
//            $ret = M('Circle')->execute($sql);
			if($ret){
				$json['status'] = '1';
				$this->inserlog('直播：'.$info['title'].',ID:'.$id.' 删除成功','Flash');
				$json['info']	= '直播：'.$info['title'].' 删除成功';
			}else{
				$json['status'] = '0';
				$json['info']	= '直播：'.$info['title'].' 删除失败';
			}
	...
	public function sortlists(){
		...
		if(IS_POST){
			...
			$this->success("修改排序成功",get_up_url(U('rec_index'),'Flash'));
		}else{
			$this->error("修改排序失败");
		}
	...
	public function editnav(){
		...
		if(IS_GET){
			...
			if($lastid){
				$this->inserlog('直播：'.$data['title'].',ID:'.$id.' 修改状态成功','Flash');
				$this->success("修改状态成功",get_up_url(U('rec_index'),'Flash'));
			}else{
				$this->error("修改状态失败");
			}

...
class Order3Controller extends CommonController {
    ...
    public function index3(){
        $this -> assign('cat_name','录播');

        create_up_url(U('index3'),'Order3');
        $_db = M('Order');
        $map = array();
        $is_service = 0;
        ...
        if($title){
            ...
            $act_title = _Selects(M('Flashrec'),$map_act,'','id desc');
            $str_act = '';
...
//		echo M()->getLastSql();
        ...
        foreach($lists as $m=>$v){
            ...
            $map_flash['id'] = array('eq',$v['aid']);
            $act_name = _Finds(M('Flashrec'),$map_flash,'');
	...
	public function editnav(){
		...
		if(IS_GET){
			...
			if($lastid){
                ...
                $this->inserlog('录播订单ID:' . $id . ' 修改处理状态成功', 'Order3');
				$this->success("修改处理状态成功",get_up_url(U('index3'),'Order3'));
			}else{
				$this->error("修改处理状态失败");
			}
    ...
    public function editshow(){
        ...
        if(IS_GET){
            ...
            if($lastid){
                ...
                $this->inserlog('录播订单ID:' . $id . ' 修改显示状态成功', 'Order3');
                $this->success("修改显示状态成功",get_up_url(U('index3'),'Order3'));
            }else{
                $this->error("修改显示状态失败");
            }

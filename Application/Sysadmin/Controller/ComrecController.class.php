...
class ComrecController extends CommonController {
    ...
    public function rec_index(){
		create_up_url(U('rec_index'),'Comrec');
		$_db = M('Comment');
		$map = array();
		$is_service = 0;
		...
		if($name){
            ...
            $news_1 = M("Flashrec")->where($map_1)->select();
            $str_1 = '';
	...
	public function rec_edit(){
		...
		if(IS_POST){
			...
			if(!$_db->create($_POST)){
				...
				if($ret){
					$this->inserlog('录播评论ID:'.$id.' 修改成功','Comrec');
					$this->success("修改成功",get_up_url(U('rec_index'),'Comrec'));
				}else{
					$this->error("修改失败");
				}
			}
...
//                $lastids = $_db_1 -> where($map_cr_1) -> save($data_cr);
    ...
    public function editcom(){
        ...
        if(IS_GET){
            ...
            if($lastid){
                $this->inserlog('录播评论ID:'.$id.' 修改通过状态成功','Comrec');
                $this->success("修改通过状态成功",get_up_url(U('rec_index'),'Comrec'));
            }else{
                $this->error("修改通过状态失败");
            }
    ...
    public function editnav(){
        ...
        if(IS_GET){
            ...
            $lastid=$Mail->where($map)->save($saveda);
            if($lastid){
                $this->inserlog('录播评论ID:'.$id.' 修改显示状态成功','Comrec');
                $this->success("修改显示状态成功",get_up_url(U('rec_index'),'Comrec'));
            }else{
                $this->error("修改显示状态失败");
            }
    ...
    public function del_com(){
        if(IS_GET){
            ...
            if($ret){
                $json['status'] = '1';
                $this->inserlog('用户'.$username.'录播评论ID:'.$id.' 删除成功','Comrec');
                $json['info']	='删除成功';
            }else{
                $json['status'] = '0';
                $json['info']	= '删除失败';

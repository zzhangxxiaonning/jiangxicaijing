...
class OrderController extends CommonController {
    ...
    public function index(){
        ...
        $this -> assign('cat_name','线下活动');

	    create_up_url(U('index'),'Order');
		$_db = M('Order');
		$map = array();
		$is_service = 0;
        ...
        if($title){
            ...
            $act_title = _Selects(M('Activity'),$map_act,'','id desc');
            $str_act = '';
...
//		echo M()->getLastSql();
		...
		foreach($lists as $m=>$v){
            ...
            $act_name = _Finds(M('Activity'),$map_activity,'');
	...
	public function editnav(){
		...
		if(IS_GET){
			...
			if($lastid){
                ...
                $this->inserlog('线下活动订单ID:' . $id . ' 修改处理状态成功', 'Order');

				$this->success("修改处理状态成功",get_up_url(U('index'),'Order'));
			}else{
				$this->error("修改处理状态失败");
			}
    ...
    public function editshow(){
        ...
        if(IS_GET){
            ...
            if($lastid){
                ...
                $this->inserlog('线下活动订单ID:' . $id . ' 修改显示状态成功', 'Order');

                $this->success("修改显示状态成功",get_up_url(U('index'),'Order'));
            }else{
                $this->error("修改显示状态失败");
            }

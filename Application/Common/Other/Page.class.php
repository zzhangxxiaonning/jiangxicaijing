...
class Page{
	...
	public function shows() {
        ...
        //上一页
        $up_row  = $this->nowPage - 1;
        $up_page = $up_row > 0 ? '<span><a class="prev" href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a></span>' : '<span><a class="prev" href="javascript:void(0)">上一页</a></span>';

        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<span><a class="next" href="' . $this->url($down_row) . '">' . $this->config['next'] . '</a></span>' : '<span><a class="prev" href="javascript:void(0)">下一页</a></span>';
        ...
        if($this->totalPages > $this->rollPage && ($this->nowPage - $now_cool_page) >= 1){
            ...
            $the_first = '<span><a class="first" href="' . $this->url(1) . '">' . $this->config['first'] . '</a></span>';
        }

        //最后一页
        $the_end = '';
        if($this->totalPages > $this->rollPage && ($this->nowPage + $now_cool_page) < $this->totalPages){
            $the_end = '<span><a class="end" href="' . $this->url($this->totalPages) . '">' . $this->config['last'] . '</a></span>';
        }
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page > 0 && $this->totalPages != 1){
                    $link_page .= '<span class="on"><a href="javascript:void(0)">' . $page . '</a></span>';
                }
            }
        }

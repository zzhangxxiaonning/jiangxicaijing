...
class Pages {
    ...
    public function show() {
		...
		$theEnd .='<li></li><input name="p_page" id="p_page" type="text" value="'.$this->nowPage.'" style="width:20px;"><a><button class="btn btn-mini" value="" type="button" style="cursor:pointer;" id="ymtiaozhuan" val="'.$tzurl.'">跳转</button>';
        // 1 2 3 4 5
        $linkPage = "";
	...
	public function shows(){
		...
		// 上一页
		if ($upRow>0){
			 $pages.=   "<a href='".str_replace('__PAGE__',$upRow,$url)."' class='page'>上一页</a>";
			 
		}else{
			 $pages.=   "<a class='page '>上一页</a>";
		}
		
		//第一页
		if($this->nowPage>($adjacents+1)) {
			$pages.= "<a href='".str_replace('__PAGE__',1,$url)."'  class='page'>1</a>";
		}
		...
		for($i=$pmin; $i<=$pmax; $i++) {
			if($i==$this->nowPage) {
				...
				$pages.= "<a href='".str_replace('__PAGE__',$i,$url)."' class='page'>".$i."</a>";
			}
		}
		...
		if($this->nowPage<($this->totalPages-$adjacents)) {
			...
			$pages.= "<a href='".str_replace('__PAGE__',$this->totalPages,$url)."'  class='page'>".$this->totalPages."</a>";
		}

		// 下一页
		if ($downRow <= $this->totalPages){
			 $pages.=   "<a href='".str_replace('__PAGE__',$downRow,$url)."' class='page next  '>下一页</a>";
		}else{
			$pages.=   "<a  class='page next'>下一页</a>";
		}
		return $pages;
    }
}
